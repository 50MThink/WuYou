...
class StatusBarUtil {
     ...
     * 沉浸式状态栏
     */
    fun ImmerseBar(activity: Activity){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            val window = activity.window
    ...
    /**
     * 自适应状态栏图标颜色，如果图标颜色为黑色，状态栏背景将设置为白色，如果状态栏图标为白色，背景设置为灰色
     */
    @SuppressLint("ResourceAsColor")
    fun setAutoStatusBarColor(activity: Activity){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M){
                activity.window.decorView.systemUiVisibility = View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR
            setWindowStatusBarColor(activity,R.color.common_white)
        }else{
            if(OSUtils.isFlymeOS4Later){
                ...
                setWindowStatusBarColor(activity,R.color.common_dull_gray)
            }
        }
    }
