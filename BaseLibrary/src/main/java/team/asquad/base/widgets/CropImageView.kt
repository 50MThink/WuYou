...
class CropImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) : ImageView(context, attrs, defStyle) {
    ...
    private var matrixs = Matrix()      //图片变换的matrix
    ...
    private fun initImage() {
        ...
        //图像中点为中心进行缩放
        matrixs.setScale(scale, scale, (mImageWidth / 2).toFloat(), (mImageHeight / 2).toFloat())
        val mImageMatrixValues = FloatArray(9)
        matrixs.getValues(mImageMatrixValues) //获取缩放后的mImageMatrix的值
        ...
        matrixs.postTranslate(transX, transY)
        imageMatrix = matrixs
        invalidate()
    }
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action and MotionEvent.ACTION_MASK) {
            ...
            -> {
                savedMatrix.set(matrixs)   //以后每次需要变换的时候，以现在的状态为基础进行变换
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (mode == DRAG) {
                    ...
                    matrixs.postTranslate(event.x - pA.x, event.y - pA.y)
                    fixTranslation()
                    imageMatrix = matrixs
                } else if (mode == ZOOM) {
                    ...
                    if (newDist > 10f) {
                        ...
                        if (tScale != 0f) {
                            matrixs.postScale(tScale, tScale, midPoint.x, midPoint.y)
                            fixScale()
                            fixTranslation()
                            imageMatrix = matrixs
                        }
                    ...
                    if (b > 10) {
                        ...
                        rotation = angleA
                        matrixs.set(savedMatrix)
                        matrixs.postRotate((rotation * 180 / Math.PI).toFloat(), midPoint.x, midPoint.y)
                        imageMatrix = matrixs
                    }
                }
            }
            MotionEvent.ACTION_UP, MotionEvent.ACTION_POINTER_UP -> {
                if (mode == DRAG) {
                    ...
                    matrixs.set(savedMatrix)
                    matrixs.postRotate((90 * rotateLevel).toFloat(), midPoint.x, midPoint.y)
    ...
    private fun fixScale() {
        ...
        matrixs.getValues(imageMatrixValues)
    ...
    private fun fixTranslation() {
        ...
        matrixs.mapRect(imageRect)  //获取当前图片（缩放以后的）相对于当前控件的位置区域，超过控件的上边缘或左边缘为负
        var deltaX = 0f
        var deltaY = 0f
    ...
    private fun doubleClick(x: Float, y: Float) {
        ...
        if (curScale < mMaxScale) {
            ...
            matrixs.postScale(toScale, toScale, x, y)
        } else {
    ...
    fun getCropBitmap(expectWidth: Int, exceptHeight: Int, isSaveRectangle: Boolean): Bitmap? {
        ...
        return makeCropBitmap(srcBitmap!!, mFocusRect, imageMatrixRect, expectWidth, exceptHeight, isSaveRectangle)
    }
    ...
    companion object {
        ...
        /******************************** 图片缩放位移控制的参数  */
        private val MAX_SCALE = 4.0f  //最大缩放比，图片缩放后的大小与中间选中区域的比值
        private val NONE = 0   // 初始化
        private val DRAG = 1   // 拖拽
        private val ZOOM = 2   // 缩放
        private val ROTATE = 3 // 旋转
        private val ZOOM_OR_ROTATE = 4  // 缩放或旋转

        private val SAVE_SUCCESS = 1001  // 缩放或旋转
        private val SAVE_ERROR = 1002  // 缩放或旋转
        private val mHandler = InnerHandler()
